### HA combined sample data analysis ###
library(ArchR)
llibrary(Seurat)
library(ggplot2)
...
### selected H samples
H_samples <- c("A1HA2","A2HA4","A3HA4","B1HA4","B2HA4","C1HA4","C2HA4","C3HA2","D1HA2","D2HA2","D3HA2")
inputFiles_selected <- inputFiles[H_samples]

# addArchRChrPrefix(chrPrefix = FALSE)

# 使用createArrowFiles函数创建arrowfiles
...
projHeme1 <- ArchRProject(
  ...
  geneAnnotation = geneAnnotation,
  genomeAnnotation = genomeAnnotation,
  outputDirectory = paste0("/spatial/run_stomics/ArchR/T2T/HA_combined"),
...
data_H_combined <- readRDS("data_RNA_HR_combined.rds")
data_H_combined$cellNames <- Cells(data_H_combined)
cellsPass <- intersect(projHeme1$cellNames,data_H_combined$cellNames)

projHeme2 <- projHeme1[cellsPass, ]
projHeme2

projHeme2 <- addCellColData(projHeme2,data=as.vector(data_H_combined$CellType[gsub("A[0-9]#","_",cellsPass)]),
                            name="CellType",cells=cellsPass,force=TRUE)
projHeme2$CellType <- factor(projHeme2$CellType,levels = levels(data_H_combined))
...
markerGenes <- c("SLC17A6","SLC32A1","TBX3","FEZF1","SIM1")
