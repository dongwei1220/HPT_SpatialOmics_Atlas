### TR_combined sample data analysis ###
...
sample_list <- c("A1TR4","A2TR4","A3TR4","B1TR4","B2TR4","C1TR4","C2TR4","C3TR2","D1TR4","D2TR5","D3TR5")
...
sample_list <- c("A1TR4","A2TR4","A3TR4","B1TR4","B2TR4","C1TR4","C2TR4","C3TR2","D1TR4","D2TR5","D3TR5")
data_A1T <- readRDS("./data_RNA_A1TR4.rds")
data_A2T <- readRDS("./data_RNA_A2TR4.rds")
data_A3T <- readRDS("./data_RNA_A3TR4.rds")
data_B1T <- readRDS("./data_RNA_B1TR4.rds")
data_B2T <- readRDS("./data_RNA_B2TR4.rds")
data_C1T <- readRDS("./data_RNA_C1TR4.rds")
data_C2T <- readRDS("./data_RNA_C2TR4.rds")
data_C3T <- readRDS("./data_RNA_C3TR2.rds")
data_D1T <- readRDS("./data_RNA_D1TR4.rds")
data_D2T <- readRDS("./data_RNA_D2TR5.rds")
data_D3T <- readRDS("./data_RNA_D3TR5.rds")
DefaultAssay(data_A1T) <- "SCT"
DefaultAssay(data_A2T) <- "SCT"
DefaultAssay(data_A3T) <- "SCT"
DefaultAssay(data_B1T) <- "SCT"
DefaultAssay(data_B2T) <- "SCT"
DefaultAssay(data_C1T) <- "SCT"
DefaultAssay(data_C2T) <- "SCT"
DefaultAssay(data_C3T) <- "SCT"
DefaultAssay(data_D1T) <- "SCT"
DefaultAssay(data_D2T) <- "SCT"
DefaultAssay(data_D3T) <- "SCT"

data_list <- list(data_A1T,data_A2T,data_A3T,data_B1T,data_B2T,data_C1T,data_C2T,data_C3T,data_D1T,data_D2T,data_D3T)
var.features <- SelectIntegrationFeatures(object.list = data_list, nfeatures = 3000)

data_RNA <- merge(x=data_A1T,y=c(data_A2T,data_A3T,data_B1T,data_B2T,data_C1T,data_C2T,data_C3T,data_D1T,data_D2T,data_D3T),
                  add.cell.ids=c("A1T","A2T","A3T","B1T","B2T","C1T","C2T","C3T","D1T","D2T","D3T"))
...
data_RNA <- FindClusters(data_RNA, resolution = 0.6)
...
#cluster_colors <- RColorBrewer::brewer.pal(n=8,"Paired")
...
for(i in unique(data_RNA$orig.ident)){
    ...
    for(gene in markerGenes){
        pdf(paste0(dataName,"_tissue/Spatial_FeaturePlot_",i,"_by_",gene,".pdf"),height=8.5,width=7)
        p <- spatial_FeaturePlot(data_sample, gene, 96) + scale_colour_continuous(type = "viridis",option="D") + 
        theme(legend.position = "bottom") + labs(title = gene)
        print(p)
        dev.off()
        }
}
...
data_RNA <- RenameIdents(object = data_RNA,
            "1"="Spermatogonia", "11"="Spermatogonia", "4"="Spermatocytes", "6"="Spermatocytes",
            "12"="RSs", "3"="RSs", "5"="ESs/Sperm","8"="ESs/Sperm",
            "0"="Sertoli","10"="Sertoli", "7"="Sertoli", 
            "2"="Leydig","13"="Leydig","14"="Myoid",
            "9"="Endothelial")
...
celltype_colors <- c("#A6CDE2","#1E78B4","#F59899","#E11E26", "#F47E1F","#6A3E98","#FCBF6E","#CAB2D6")
...
DimPlot(data_RNA, reduction = "umap", label = F, split.by="Stage",label.size = 5,ncol=2, raster = F) + scale_color_manual(values=celltype_colors)

for(i in unique(data_RNA$orig.ident)){
    data_sample <- subset(data_RNA, subset= orig.ident == i )
    data_sample <- RenameCells(data_sample,new.names = gsub("^.*_","",Cells(data_sample)))
    pdf(paste0(dataName,"_tissue/Spatial_DimPlot_umap_",i,"_by_celltype.pdf"),height=7,width=9.7)
    p <- spatial_DimPlot(data_sample, reduction = "umap", 96) + scale_color_manual(values=celltype_colors) + 
...
data_RNA@meta.data[data_RNA@meta.data$orig.ident %in% c("A1TR4","A2TR4","A3TR4"),"Stage"] <- "Juvenile_Testis"
data_RNA@meta.data[data_RNA@meta.data$orig.ident %in% c("B1TR4","B2TR4"),"Stage"] <- "Pubertal_Testis"
data_RNA@meta.data[data_RNA@meta.data$orig.ident %in% c("C1TR4","C2TR4","C3TR2"),"Stage"] <- "Adult_Testis"
data_RNA@meta.data[data_RNA@meta.data$orig.ident %in% c("D1TR4","D2TR5","D3TR5"),"Stage"] <- "Old_Testis"
data_RNA$Stage <- factor(data_RNA$Stage,levels = c("Juvenile_Testis","Pubertal_Testis","Adult_Testis","Old_Testis"))
